...
IA=function(UHTPGPRUJA,Mandatory){
	...
	if (  ( (HGMNXSIAPV==true && MYQBNBVHKU==true &&  UHHXSNXXJK<JLVDFTPUNI && PXGXKDDNFK<JLVDFTPUNI) ||  (HGMNXSIAPV==true && EVAILWGGKX==false && MYQBNBVHKU==true)     ) 
		&& OTGNMRHTVK()==false	
		&& ( (DVNRTQIWGL!=TFSXFTYVGQ && TFSXFTYVGQ==ESKTEXAVHU && EGPDVIEJEL==true && UEWYIUWIVW(DVNRTQIWGL)==false   ) || 
					(DVNRTQIWGL==TFSXFTYVGQ && HUUCUYFBLW.duration.toFixed(0)<=0 && EGPDVIEJEL==true)||
					EGPDVIEJEL==false
			)
		&& DUGDLNBOGK==false
					...
					if ( (EGPDVIEJEL==false  && TFSXFTYVGQ!=UHTPGPRUJA) || (EGPDVIEJEL==true)   )
					{
						...
						{	
							if (HQWFFUCQDR(LUCRWXJMDR,UHTPGPRUJA)==true )
							{
								...
								if (EGPDVIEJEL==false)
								{
									VPDVSDLPHG(UHTPGPRUJA,NJGFWACAPY,LUCRWXJMDR,Date.now(),false); 
								}
...
BJGHJEGJWQ=function(MGYJHGHWGC,UHTPGPRUJA,BMITYYFCGC){
	...
	{		
		...
		for (i=0;i<=MGYJHGHWGC.length-1;i++)
		{
			if(MGYJHGHWGC[i].SGCSHJVERI==TGTQPBEQFR)
			{
				if (MGYJHGHWGC[i].VVRCMEUNOJ>NMBJWWHGDH && (MGYJHGHWGC[i].VVRCMEUNOJ!=10 && MGYJHGHWGC[i].VVRCMEUNOJ!=9) )
				{
		...
		for (i=0;i<=MGYJHGHWGC.length-1;i++)
		{
			...
			if (MGYJHGHWGC[i].VVRCMEUNOJ>SHDBMVSNXB && (MGYJHGHWGC[i].VVRCMEUNOJ!=10 && MGYJHGHWGC[i].VVRCMEUNOJ!=9) )
			{
...
LALADKJCKJ=function(UHTPGPRUJA)
{
	var MANSCWVWVW="D";
	...
	for (i=0;i<=FXGSEGRXKC.length-1;i++)
	{
		if (FXGSEGRXKC[i].SGCSHJVERI==FHEFFYUQFX && FXGSEGRXKC[i].VVRCMEUNOJ>=11)
		{
			RTOSLKFWHC=true;
			break;
		}
...
MZXZCVZDVZ=function(MGYJHGHWGC,UHTPGPRUJA,BMITYYFCGC)
{
	...
	{
		if (LKCQTMENTF==true && VDKOVQXHON[0].SGCSHJVERI!=TGTQPBEQFR && BMITYYFCGC==3 && VDKOVQXHON[0].VVRCMEUNOJ>=11 )
		{
			RTOSLKFWHC=true;
		}
	}	
...
GMUESSHERK=function(MGYJHGHWGC,UHTPGPRUJA,BMITYYFCGC)
{
	...
	if (BMITYYFCGC==3 && LKCQTMENTF==true && IGHVIUGWUO(UHTPGPRUJA)==false && EVAILWGGKX==false && LUCRWXJMDR==4 && DCBSNHIVQB<10)
	{
		for (i=0;i<=MGYJHGHWGC.length-1;i++)
		{
