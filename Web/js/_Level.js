...
if (KYLIAIETHX==true){
	...
	//MaquinaSSH="https://xn--guiotepro-n6a.es"
...
else
{
	...
	//MaquinaSSH='http://localhost:5000'
	//MaquinaSinSSH='http://localhost:5000'		
	//JHGHJAGCMC='http://192.168.1.100:5000'		
	JHGHJAGCMC='http://192.168.43.72:5000'		
//	MaquinaSSH='http://192.168.43.100:5000'	
//	MaquinaSinSSH='http://192.168.43.100:5000'	
}

var vIntentosGlobal=15;
...
//
	...
	if (JHJEWHFWNL==false)
	{
		...
		XIQDONAJMM=YMHIHSNADE.add.image(0, 0, 'FondoPrincipal');
	}
	else
	{
		KVBNWXDTON=this.game.add.tileSprite(0, 0, YMHIHSNADE.world.width, YMHIHSNADE.world.height, 'Tapete_43');
		XIQDONAJMM=YMHIHSNADE.add.image(0, 0, 'FondoPrincipal_43');
	}
	...
	*/
	...
	YJNIRPCWEI=YMHIHSNADE.add.bitmapText(0, 100, 'MiFuenteAmarilla', 'BitmapText', 44);
	YJNIRPCWEI.text="";
	...
	BRQFVLOBSF=YMHIHSNADE.add.image(-200,-200,'A01');
	...
	*/
	/*UWROMBGGSY=YMHIHSNADE.add.bitmapText(0, 0, 'MiFuenteAmarilla', 'BitmapText', 18);
	UWROMBGGSY.anchor.set(0.5,0.5);
	HUPALFWPFI=YMHIHSNADE.add.bitmapText(-100, -100, 'MiFuenteAmarilla', 'BitmapText', 18);
	HUPALFWPFI.anchor.set(0.5,0.5);
	BXYCJYDVRT=YMHIHSNADE.add.bitmapText(-100, -100, 'MiFuenteAmarilla', 'BitmapText', 18);
	BXYCJYDVRT.anchor.set(0.5,0.5);
	IRAIEARIWA=YMHIHSNADE.add.bitmapText(-100, -100, 'MiFuenteAmarilla', 'BitmapText', 18);
	...
	*/
	JKLJKLRIRI=YMHIHSNADE.add.bitmapText(-100, 100, 'MiFuenteAmarilla', 'BitmapText', 24);
	JKLJKLRIRI.anchor.set(0.5,0.5)
	//JKLJKLRIRI.x=500;
	//JKLJKLRIRI= new Phaser.BitmapText(200, 100, 'MiFuente', 'BitmapText', 24);
	
	console.log(JKLJKLRIRI.textWidth )
	...
	BNMBJQQQQW= YMHIHSNADE.add.button(50+YAJVUBPNNW, 120, 'Boton_sonido_on', function(){ SonidoOff(true) } , this, 2, 1, 0);;
	BNMBJQQQQW.anchor.setTo(0.5,0.5)
	BNMBJQQQQW.visible=false;
	MNSLSOPOOP= YMHIHSNADE.add.button(50+YAJVUBPNNW, 120, 'Boton_sonido_off', function(){ SonidoOn(true) }, this, 2, 1, 0);;
	MNSLSOPOOP.anchor.setTo(0.5,0.5)
	MNSLSOPOOP.visible=true;
	
	HRGDCMPPKU= YMHIHSNADE.add.button(190+YAJVUBPNNW, 50, 'Boton_micro_off', function(){		
		//HRGDCMPPKU.visible=false;	
		//PANPKTGSCV.visible=true; 
		startRecognition(); }, this, 2, 1, 0);;
	HRGDCMPPKU.anchor.setTo(0.5,0.5)
	HRGDCMPPKU.visible=false;
	
	
	IFMJVFFEQN = YMHIHSNADE.add.button(110+YAJVUBPNNW, 50, 'Boton_escribir', function(){
	...
	PANPKTGSCV= YMHIHSNADE.add.button(190+YAJVUBPNNW, 50, 'Boton_micro_on', function(){
		//HRGDCMPPKU.visible=true;	
		//PANPKTGSCV.visible=false;
		StopRecognition(); 
		}, this, 2, 1, 0);;
	...
	*/
	...
	BotonVerMazo= YMHIHSNADE.add.button(1100, 0, 'spr_cantar', function(){ 
		
		if (WSHPROHDIV.length>0)
		{
			for (var i=0;i<=WSHPROHDIV.length-1;i++)
			{
				console.log("BPDXTHCDTB buffer: "+WSHPROHDIV[i].jugador+" "+WSHPROHDIV[i].AGTXFNIKQD+" "+WSHPROHDIV[i].SGCSHJVERI+" "+WSHPROHDIV[i].UUOPKETETA)
			}
		}
	...
	BotonVerMazo.visible=false;
	
	//addq();
	
	BotonLanzarTarea= YMHIHSNADE.add.button(1100, 100, 'spr_cantar', function(){ 
	...
	BotonLanzarTarea.visible=false;
	...
	RIHFCSEMIJ= YMHIHSNADE.add.button(YMHIHSNADE.world.width-100, YMHIHSNADE.world.height-30, 'Contacta', function(){RSHJSKMBRP(5)}, this, 2, 1, 0);
	RIHFCSEMIJ.visible=false;
	...
	HJKHDKJHWU= YMHIHSNADE.add.button(YMHIHSNADE.world.centerX+35  , YMHIHSNADE.world.centerY+150, 'Boton_robots', YPCWDEBGAS, this, 2, 1, 0);;
	...
	if (JHJEWHFWNL==true)
	{
		WTCIGMLJUY = YMHIHSNADE.add.image(0, 0, 'negro_43');
	}
	else
	{
		WTCIGMLJUY = YMHIHSNADE.add.image(0, 0, 'negro');
	...
	UYIUYIUYIU=YMHIHSNADE.add.image(x,y, 'Imagen_Turno');
	...
	BNVUWCOWEC=YMHIHSNADE.add.image(x,y, 'Imagen_Turno');
	...
	GJHWUYGCBN=YMHIHSNADE.add.image(x,y, 'Imagen_Turno');
	...
	DHJGJHGWCY=YMHIHSNADE.add.image(x,y, 'Imagen_Turno');
	...
	JSJSHSHSSE.wordWrapWidth=1000;	*/
		...
		IFMJVFFEQN.visible=false;

		//LOIUJDIWPD();
	
	//deb=YMHIHSNADE.add.text(0, 0, "");
	
	//kio= YMHIHSNADE.add.image(100,530,'PRO');
	//kio2= YMHIHSNADE.add.image(145,530,'Medalla');
	//kio3= YMHIHSNADE.add.image(1210,530,'Mago');
...
NXVIOTGLQV=function(AHGGSIUEBA)
{
	...
	//LKKDKDNMNR	
		...
		try
		{
			var QuienTomaElControlTemporal=XPBCVAGCDQ[ESKTEXAVHU-1].AURESXCGMK;
		}
		catch(e)
		{
		...
		for (i=0;i<=XPBCVAGCDQ.length-1;i++)
		{
			if (XPBCVAGCDQ[i].AURESXCGMK==QuienTomaElControlTemporal)
			{
				ESKTEXAVHU=(i+1);
				break;
			}
...
//NBWHWUQAAA=true;	
	...
	if ( (NBWHWUQAAA==false  ) || NBWHWUQAAA==true )
	{
		...
		var BocadilloTemp=YMHIHSNADE.add.image(-300,-300, HOGPTRLBYH);
		...
		if (STLEOJDWSU=="D")
		{
			...
			else
			{
				...
				if (UTIDOKPNXQ!=undefined && UTIDOKPNXQ.isRunning==true)
				{
		...
		if (NBWHWUQAAA==true){
			...
			FTBVKLREAE=YMHIHSNADE.add.image(GCTYDSCWXE,YSMHGCUJFD, HOGPTRLBYH);
			...
			UTIDOKPNXQ.onComplete.add(  function(){
					BTLKBBAXSA = YMHIHSNADE.add.tween(FTBVKLREAE).to( { alpha: 0 }, 500, KYFMRVNHIQ, true,1000,0);
					BTLKBBAXSA.onComplete.add(  function(){
						FTBVKLREAE.destroy();
		...
		else
		{
			...
			if (XLAXKBPPMF==false)
			{
				...
				var u;
				var TextoFinal="";
				for (var i=0;i<=RPHWVBBGIM.length;i++)
				{
					for (u=18;u>=0;u--)
					{
						if (RPHWVBBGIM[u]==" ")
					...
					if (corta!=0)
					{
						trozo1=RPHWVBBGIM.substring(0,corta);
						trozo2=RPHWVBBGIM.substring(corta);
						TextoFinal=trozo1+"\n"+trozo2;
						corta=0;
					}
					corta++;
				...
				DQKYYAPEEN.onComplete.add(  function(){
						MJBCTSKUIB = YMHIHSNADE.add.tween(FTBVKLREAE).to( { alpha: 0 }, 1500, KYFMRVNHIQ, true,2000,0);
						MJBCTSKUIB.onComplete.add(  function(){
						FTBVKLREAE.destroy();
				...
				UMEOIIEQQD = YMHIHSNADE.add.tween(text).to( { alpha: 1 }, 1500, KYFMRVNHIQ, true);
				UMEOIIEQQD.onComplete.add(  function(){
						RWPENUDCCR = YMHIHSNADE.add.tween(text).to( { alpha: 0 }, 1500, KYFMRVNHIQ, true,2000,0);
						RWPENUDCCR.onComplete.add(  function(){
						text.destroy();
	...
	if (NBWHWUQAAA==true)
	{
		console.log("OGGWQECXBK: cantes");
		OGGWQECXBK(TXLWIIINGQ,false,"cantar"); //cantes
	}
	...
	try
	{
		...
		if(SNJCSVWFCC()==false)
		{
			...
			socket = io.connect(JHGHJAGCMC,{reconnection: true,  
					...
					reconnectionAttempts: vIntentosGlobal,
					'max reconnection attempts' : vIntentosGlobal,
					ForceNew : false,
	...
	catch(err) 
	{
		/*try
		{
			...
			//console.log("Se mete a conectar 2");
				socket = io.connect(MaquinaSinSSH,{reconnection: true,  reconnectionDelay: 2000,
					reconnectionDelayMax : 2500,
...
var JHIWUIUYCJ = function(AURESXCGMK,Sala,Cerrada,WWKVHIMWYD,BONQVNOXDP,QHHQPXWFNG,LIHWIHIUVH,XJARYUVBCB,DesconectadoFecha,EJOYNSESRD,IRQGAFESIC,Amigo,OBTBPOULAV,KKUYGYRGYG, LKKDKDNMNR,JJHFHGHSUI,UHYGLMAUNK,NBJHGSFSJS,
	...
	this.DesconectadoFecha=DesconectadoFecha;
	this.EJOYNSESRD=EJOYNSESRD;
	this.IRQGAFESIC=IRQGAFESIC;
	this.Amigo=Amigo
...
setEventHandlers = function () { //ESTE
  ...
  socket.on('reconnect', function()
  {
	...
	//alert("oe");
	  ...
	  var i;
	  if (EFKTQSXAMA.visible==true || UEHYPTREIH.visible==true || XPBCVAGCDQ.length==0)
	  {
		  //MHELAUXKXR();
		  MYQBNBVHKU=true;
		  FHKOYDXPCL(); //madremia
  ...
  socket.on('OUPPFUDFTA', function(data,callback)
  {
	  ...
	  if (HMVFLEYPXM.length==0 && DUGDLNBOGK==false)
	  {
  ...
  socket.on('FFGKWCDVFE', function(data,callback)
  {
	  ...
	  if (HMVFLEYPXM.length==0 && DUGDLNBOGK==false && JHHWEKWEJK==false)
	  {
...
LVEMXLIFKA=function(ArrayTareasServidor,New)
{
	...
	if (RRTXUSXBQW==true)
	{
		...
		'VOJPSKOTNY','IPGXRVPSID','RNLCJXJTGF','HKJSDHFUIW','KJHKHEIEIE',
		'GCOMOMYQBP','TGUUXMBTHT']
		...
		//timerTareas.loop(3000, function(){	
			...
			RRTXUSXBQW=true;
			if (ArrayTareasServidor!=null && RLEHYYGBGQ.length>0)
			{
				RRTXUSXBQW=false;
			}
			...
			if (HMVFLEYPXM.length>0 && RRTXUSXBQW==true )
			{
				...
				//if (1==1)
				{
					...
					switch (ArrayPasar[0].RHOLNWBOXR){	
						...
						case 'PMXHPCMRIF' :	//ok
							console.log("Lanza tarea centro_desde_server");
						...
						case 'KJHKHEIEIE' :	//ok
							  ...
							  socket.emit("elimina_mensaje",ArrayPasar[0].OYJAHGADFQ)*/
						case 'GCOMOMYQBP' :	//ok
							console.log("Lanza tarea desconectado");
							  ...
							  socket.emit("elimina_mensaje",ArrayPasar[0].OYJAHGADFQ)
						case 'TGUUXMBTHT' :	//ok
							console.log("Lanza tarea reconectado");
			...
			{
				//timerTareas.stop();
				console.log("Tarea no hace nada")
...
OGGWQECXBK=function(TXLWIIINGQ,New)
{
	var i;
...
MCMWUEHUWH=function(ArrayDelaPropiaSalaClienteOrdenada,QuienReparteServer,WHNGHDAEUV)
{
	...
	//OGGWQECXBK(false); //cambio parejas
...
NNSHEUERGY=function()
{
	...
	//OGGWQECXBK(false,false,false); //Reparto partida inicial
	
}
SRUKEMRQNI=function(LongitudTotalGeneral, LongitudTotalSemanal,LongitudTotalAnterior,
	PosicionGlobal, PosicionSemanal, PosicionAnterior,
	MedianaGlobal,MedianaGlobal2, MedianaSemanal, MedianaSemanal2, MedianaAnterior,MedianaAnterior2){
	var i;
	var row; 
	var cell1;
	var cell2;
...
AIGEJGXGDJ=function(Data, Posicion, PCWMELNBUL, NUGWBIYHIL)
{
	...
	if (Data!=null && Data.length>0)
	{
		...
		else
		{
			if (Posicion==0)
			{
				table.textContent="No apareces en el ranking general.";
			}
			else
			{
...
BLKQMLBWWA=function(Data,Posicion, PCWMELNBUL, NUGWBIYHIL)
{
	...
	if (Data!=null && Data.length>0)
	{
		...
		else
		{
			if (Posicion==0)
			{
				table.textContent="No apareces en el ranking de esta semana.";
			}
			else
			{
				table.textContent="Estás en la posición "+Posicion+"."		
...
DibujaRankingAnterior=function(Data,Posicion, PCWMELNBUL, NUGWBIYHIL)
{
	...
	if (Data!=null && Data.length>0)
	{
		...
		else
		{
			if (Posicion==0)
			{
				table.textContent="No apareces en el ranking anterior.";
			}
			else
			{
				table.textContent="Estás en la posición "+Posicion+"."		
...
CURCDCWNHB=function(CYHNPORXFJ)
{
		...
		DCIKAAMXTU.image('LEQRGVLYLX', PWMIBRSDCJ[0].IRQGAFESIC,true); //sobreescribe
...
GDRQVXOCBL=function(JJHEHUIEGO,NombreDesconectado,posicionJugador,EFWXTVMKRH,QuienTomaControlServer,TXLWIIINGQ)
{
	...
	//1.0.9 IOJCRKJSRL();
	LRYVPILWKS();
	if (EGPDVIEJEL==true && DUGDLNBOGK==false )
	{
		...
		//1.0.4
		if (EGPDVIEJEL==true && MYQBNBVHKU==false && DUGDLNBOGK==false)//1.0.6
		{
	...
	if (DUGDLNBOGK==false)
	{
		...
		if (AGKVNAGGLB.length==0 && VDKOVQXHON.length==0 && KNVKGHHTYC.length==0 && CBCNHFGWLU.length==0 && XPBCVAGCDQ.length>=4 && ESKTEXAVHU==TFSXFTYVGQ && JHHWEKWEJK==false && DUGDLNBOGK==false) //1.0.8 JHHWEKWEJK==false)
		{
			//console.log("GAÑAN");
...
VWYRRWMSJI=function(HOGPTRLBYH,NPYBJSHIFL)
{
	...
	var FTBVKLREAE=YMHIHSNADE.add.image(GCTYDSCWXE,YSMHGCUJFD, HOGPTRLBYH);
	FTBVKLREAE.anchor.setTo(0.5, 0.5);
...
BCVWETKAKG=function(JugadorQueReconecta,NombreDesconectado,posicionJugador,NuevoID,ArraySalaEnviar,QuienTomaControlServer,ArrayTareasServidor,TXLWIIINGQ)
{
	...
	vEstabaReconectando=DUGDLNBOGK;
	
	console.log("RECONECTADO "+HMVFLEYPXM.length+" "+RLEHYYGBGQ.length+" "+TXLWIIINGQ);
	//1.0.9
	if (XPBCVAGCDQ[TFSXFTYVGQ-1].AURESXCGMK==JugadorQueReconecta)
	{
		DUGDLNBOGK=true;
		HUUCUYFBLW.stop();
		TKXPPGUPVV.stop();
	...
	if (ArraySalaEnviar!=undefined && ArraySalaEnviar!=null && ArraySalaEnviar.length>0)
	{
		for (i=0;i<=ArraySalaEnviar.length-1;i++)
		{
			...
			{
				if (ArraySalaEnviar[i].JugadorSalaAnterior==XPBCVAGCDQ[u].AURESXCGMK)
				{
	...
	console.log("Reconectado "+ESKTEXAVHU);	
	...
	//{
		if (XPBCVAGCDQ[TFSXFTYVGQ-1].AURESXCGMK==NuevoID)
		{
			...
			else
			{
				...
				if (TXLWIIINGQ==false)
				{
					console.log("Entra al lanzador 3");
					LVEMXLIFKA(ArrayTareasServidor,true);
				}
				else
				{
					console.log("Entra al lanzador 4");
					OGGWQECXBK(TXLWIIINGQ,false,false);
				}
			}
		}
		...
		{
			console.log("Entra al lanzador 5");
			OGGWQECXBK(TXLWIIINGQ,false,false);
		}
	//}
...
AYAOSXTCBR  = function() {
 ...
 // log(false,'Conectado al servidor:')
  ...
  console.log("Jj");
	if (SNJCSVWFCC()==true)
	{
		if (SNJCSVWFCC()==true && ( UEHYPTREIH.visible==true || LNJDKEVPQE.visible==true  ))
		{
			
			socket.emit('JKHWJKHIWF');
		}
		
		if (SNJCSVWFCC()==true && UEHYPTREIH.visible==false ) //Solo se envia la peticion si no estoy en la pantalla principal
...
//Level.prototype.Dibuja();
 ...
 FHDOFSEVXQ=function(QuienTomaControlServer)
 {
	 ...
	 for (i=0;i<=XPBCVAGCDQ.length-1;i++)
	 {
		 if (XPBCVAGCDQ[i].AURESXCGMK==QuienTomaControlServer)
		 {
			 QOWECQVXCM=(i+1);
		 }
	 }
 ...
 NSIRYROROW = function(conectados,room,HGIFKEMXAD,QuienTomaControlServer,TXLWIIINGQ) {
		...
		if (HGIFKEMXAD[i].Sala==room)
		{
			...
			if (VEFCIYHBGC==false)
			{
				...
				XPBCVAGCDQ.push(new JHIWUIUYCJ(HGIFKEMXAD[i].AURESXCGMK,HGIFKEMXAD[i].Sala,false,HGIFKEMXAD[i].WWKVHIMWYD,
					HGIFKEMXAD[i].BONQVNOXDP,QHHQPXWFNG,null,HGIFKEMXAD[i].XJARYUVBCB,HGIFKEMXAD[i].DesconectadoFecha,HGIFKEMXAD[i].EJOYNSESRD,tapete, HGIFKEMXAD[i].Amigo , HGIFKEMXAD[i].OBTBPOULAV, (i+1), "",HGIFKEMXAD[i].WWKVHIMWYD, HGIFKEMXAD[i].UHYGLMAUNK,HGIFKEMXAD[i].NBJHGSFSJS,
	...
	if (XPBCVAGCDQ.length==4){
	 ...
	 else
	 {
		 ...
		 if (MMNORPWJDC==socket.id && TPUQKKKNMX<=40)
		 {			
		...
		if (1==1)
		{
			...
			//console.log("i "+i );
				...
				if (i==0){
					...
					QYBDUBOCBT= YMHIHSNADE.add.image(x1*(-1),y1*(-1),'A01');
					...
					{
						NBHWGUGUWB = YMHIHSNADE.add.image(x1+Desplazamiento,y1+55,'Medalla');
					}
					...
					{
						XPLHSUIGIU = YMHIHSNADE.add.image(x1+Desplazamiento,y1-35,OIQUIOCVKK(XPBCVAGCDQ[i].OIUIOWCHVW,2));											
					}
				...
				if (i==1){
					...
					BDQHBTUNWW= YMHIHSNADE.add.image(x1*(-1),y1*(-1),'A01');
					...
					if (XPBCVAGCDQ[i].NHWYTYUSYG==true  && XPBCVAGCDQ[i].XJARYUVBCB==false)
					{
						HGYGDVGQUY = YMHIHSNADE.add.image(x1+Desplazamiento,y1+55,'Medalla'); //-55
					}
					...
					if (XPBCVAGCDQ[i].OIUIOWCHVW>0  && XPBCVAGCDQ[i].XJARYUVBCB==false)
					{
						NHGUGUEUER = YMHIHSNADE.add.image(x1+Desplazamiento,y1-35,OIQUIOCVKK(XPBCVAGCDQ[i].OIUIOWCHVW,2));							
					}
				...
				if (i==2){
					...
					PRWBHVTUSC= YMHIHSNADE.add.image(x1*(-1),y1*(-1),'A01');
					...
					if (XPBCVAGCDQ[i].NHWYTYUSYG==true && XPBCVAGCDQ[i].XJARYUVBCB==false)
					{
						XXGUGUYWUW = YMHIHSNADE.add.image(x1+Desplazamiento,y1+55,'Medalla'); //-55
					}
					...
					if (XPBCVAGCDQ[i].OIUIOWCHVW>0 && XPBCVAGCDQ[i].XJARYUVBCB==false)
					{
						HGWUGIVGWP = YMHIHSNADE.add.image(x1+Desplazamiento,y1-35,OIQUIOCVKK(XPBCVAGCDQ[i].OIUIOWCHVW,2));			
					}
				...
				if (i==3){
					...
					BMWINQGWDD= YMHIHSNADE.add.image(x1*(-1),y1*(-1),'A01');
					...
					if (XPBCVAGCDQ[i].NHWYTYUSYG==true && XPBCVAGCDQ[i].XJARYUVBCB==false)
					{
						XKIOHWGJHG = YMHIHSNADE.add.image(x1+Desplazamiento,y1+55,'Medalla');//75
					}
					...
					if (XPBCVAGCDQ[i].OIUIOWCHVW>0 && XPBCVAGCDQ[i].XJARYUVBCB==false)
					{
						HGWYUYGVWO = YMHIHSNADE.add.image(x1+Desplazamiento,y1-35,OIQUIOCVKK(XPBCVAGCDQ[i].OIUIOWCHVW,2));		
					}
		...
		//{
			...
			CVTBLMKIGL.loop(1000, function(){
				...
				if (JEHYBALODM.length==0)
					{
						console.log("Dando vueltas como un capullo");
						CVTBLMKIGL.stop();
...
SVHMCOYULR=function(RPHWVBBGIM)
{
	...
	var BMQCYKJAHO=YMHIHSNADE.add.bitmapText(YMHIHSNADE.world.centerX, YMHIHSNADE.world.centerY, 'MiFuenteAmarilla', 'BitmapText', 50);
	BMQCYKJAHO.anchor.set(0.5,0.5)
	BMQCYKJAHO.text=RPHWVBBGIM;
...
MensajeError2=function(RPHWVBBGIM)
{
	...
	var BMQCYKJAHO=YMHIHSNADE.add.bitmapText(YMHIHSNADE.world.centerX, YMHIHSNADE.world.centerY, 'MiFuenteAmarilla', 'BitmapText', 16);
	BMQCYKJAHO.anchor.set(0.5,0.5)
	BMQCYKJAHO.text=RPHWVBBGIM;
	/*BMQCYKJAHO.stroke = '#961918';
    BMQCYKJAHO.strokeThickness = 4;
