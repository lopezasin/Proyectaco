...
else
{
	...
	//JHGHJAGCMC='http://192.168.1.43:5000'		
...
//console.log("F2");
	...
	{
		MUIUIFIUWE=YMHIHSNADE.add.image(0,0, 'general', 'bocadillo_d');
	}
...
*/
	...
	{
		//POEICHMHDL.destroy();
		POEICHMHDL= YMHIHSNADE.add.button(YMHIHSNADE.world.centerX + 420, YMHIHSNADE.world.centerY-100, 'general', SolicitaClasificacion, this, 'boton_ranking','boton_ranking','boton_ranking');
...
NXVIOTGLQV=function(AHGGSIUEBA)
{
	...
	//LKKDKDNMNR	
		...
		for (i=0;i<=XPBCVAGCDQ.length-1;i++)
		{
			if (XPBCVAGCDQ[i].AURESXCGMK==JKGHWJKGJW)
			{
				ESKTEXAVHU=(i+1);
				break;
			}
...
CURCDCWNHB=function(CYHNPORXFJ)
{
		...
		if (UEHYPTREIH.alive==false) //1.0.8 El IF solamente. Resulta que según la rapidez de calculaperfil se superponían imágenes de los botones
		{
			YBWXJFDOAW();
		}
...
GDRQVXOCBL=function(JJHEHUIEGO,NombreDesconectado,posicionJugador,EFWXTVMKRH,QuienTomaControlServer,TXLWIIINGQ)
{
	...
	if (JJHEHUIEGO!=XPBCVAGCDQ[TFSXFTYVGQ-1].AURESXCGMK && JJHEHUIEGO!=XPBCVAGCDQ[TFSXFTYVGQ-1].JugadorSalaAnterior)
	{
		if (KYLIAIETHX==false)
		{
		...
		for (var i=0;i<=XPBCVAGCDQ.length-1;i++)
		{
			...
			if (XPBCVAGCDQ[i].AURESXCGMK==JJHEHUIEGO || XPBCVAGCDQ[i].JugadorSalaAnterior==JJHEHUIEGO)
			{
	...
	if (ArraySalaEnviar!=undefined && ArraySalaEnviar!=null && ArraySalaEnviar.length>0)
	{
		for (i=0;i<=ArraySalaEnviar.length-1;i++)
		{
			...
			for (u=0;u<=XPBCVAGCDQ.length-1;u++)
			{
				...
				{
					XPBCVAGCDQ[u].JugadorSalaAnterior=XPBCVAGCDQ[u].AURESXCGMK; //dangerous
		...
		if (HGIFKEMXAD[i].Sala==room)
		{
			...
			if (VEFCIYHBGC==false)
			{
				...
				XPBCVAGCDQ.push(new JHIWUIUYCJ(HGIFKEMXAD[i].AURESXCGMK,HGIFKEMXAD[i].Sala,false,HGIFKEMXAD[i].WWKVHIMWYD,
					HGIFKEMXAD[i].BONQVNOXDP,QHHQPXWFNG,null,HGIFKEMXAD[i].XJARYUVBCB,HGIFKEMXAD[i].DesconectadoFecha,HGIFKEMXAD[i].EJOYNSESRD,HGIFKEMXAD[i].IRQGAFESIC, HGIFKEMXAD[i].Amigo , HGIFKEMXAD[i].OBTBPOULAV, (i+1), "",HGIFKEMXAD[i].WWKVHIMWYD, HGIFKEMXAD[i].UHYGLMAUNK,HGIFKEMXAD[i].NBJHGSFSJS,
		...
		if (1==1)
		{
			...
			////console.log("i "+i );
				...
				if (i==0){
					...
					QYBDUBOCBT.frameName='imagen_turno'
					QYBDUBOCBT.x=x1*(-1);
					QYBDUBOCBT.y=y1*(-1);
					QYBDUBOCBT.revive();
					...
					if (XPBCVAGCDQ[i].NHWYTYUSYG==true && XPBCVAGCDQ[i].XJARYUVBCB==false)
					{
						...
						NBHWGUGUWB.x=x1+Desplazamiento;
						NBHWGUGUWB.y=y1+55;
						NBHWGUGUWB.revive();
					}
				...
				if (i==1){
					...
					BDQHBTUNWW.frameName='imagen_turno'
					BDQHBTUNWW.x=x1*(-1);
					BDQHBTUNWW.y=y1*(-1);
					BDQHBTUNWW.revive();					
				...
				if (i==2){
					...
					*/
					...
					PRWBHVTUSC.frameName='imagen_turno'
					PRWBHVTUSC.x=x1*(-1);
					PRWBHVTUSC.y=y1*(-1);
					PRWBHVTUSC.revive();		
					...
					if (XPBCVAGCDQ[i].NHWYTYUSYG==true && XPBCVAGCDQ[i].XJARYUVBCB==false)
					{
						...
						XXGUGUYWUW.x=x1+Desplazamiento;
						XXGUGUYWUW.y=y1+55;
						XXGUGUYWUW.revive();
					}
					...
					if (XPBCVAGCDQ[i].OIUIOWCHVW>0 && XPBCVAGCDQ[i].XJARYUVBCB==false)
					{
						...
						HGWUGIVGWP.x=x1+Desplazamiento;
						HGWUGIVGWP.y=y1-35;
						HGWUGIVGWP.revive();
					}
				...
				if (i==3){
					...
					*/
					...
					BMWINQGWDD.frameName='imagen_turno'
					BMWINQGWDD.x=x1*(-1);
					BMWINQGWDD.y=y1*(-1);
					BMWINQGWDD.revive();	
					...
					if (XPBCVAGCDQ[i].NHWYTYUSYG==true && XPBCVAGCDQ[i].XJARYUVBCB==false)
					{
						...
						XKIOHWGJHG.x=x1+Desplazamiento;
						XKIOHWGJHG.y=y1+55;
						XKIOHWGJHG.revive();
					}
					...
					if (XPBCVAGCDQ[i].OIUIOWCHVW>0 && XPBCVAGCDQ[i].XJARYUVBCB==false)
					{
						...
						HGWYUYGVWO.x=x1+Desplazamiento;
						HGWYUYGVWO.y=y1-35;
						HGWYUYGVWO.revive();						
					}
...
MensajeError2=function(RPHWVBBGIM)
{
	...
	if (YMHIHSNADE!=undefined)
	{
		...
		var TextoGeneral=YMHIHSNADE.add.bitmapText(YMHIHSNADE.world.centerX, YMHIHSNADE.world.centerY-25, 'MiFuenteAmarilla', 'BitmapText', 16);
		TextoGeneral.anchor.set(0.5,0.5)
		TextoGeneral.text=RPHWVBBGIM;
