...
//
	...
	//PhaserInput.focusOut.addOnce(function() { alert("ss"); }); 
...
JTQKCQEDPC=function()
{
	...
	/*var ref = cordova.InAppBrowser.open('http://xn--guiotepro-n6a.es/auth/google', '_blank', 'location=no');
	var myCallback = function(event) { alert(event.url); }
	ref.addEventListener('loadstart', myCallback);
	ref.removeEventListener('loadstart', myCallback);
	...
	log(true,"CONTROL MIERDER: "+ESKTEXAVHU);
	log(true,"DVNRTQIWGL MIERDER: "+DVNRTQIWGL);
	
	
	//timerDesconectados.stop();	
	/*socket.disconnect();
	
  
	addquake();
	
	log(true,"CONTROL: "+ESKTEXAVHU);
	log(true,"DVNRTQIWGL: "+DVNRTQIWGL);
	LRYVPILWKS();
	console.log("DESCONETADOS "+KYDWXPGHVY.length);
	for (var i=0; i<=KYDWXPGHVY.length-1;i++)
	{
		console.log("UHYCGYFWYJ desde detener tiempo "+KYDWXPGHVY[i].SSUNXNEEHN);
	}*/
	//DVNRTQIWGL*/
}
...
//EsCante=true;	
	...
	if ( (EsCante==false  ) || EsCante==true )
	{
		...
		BocadilloTemp=YMHIHSNADE.add.sprite(-300,-300, HOGPTRLBYH);
		...
		if (STLEOJDWSU=="D")
		{
			if (EsCante==false)
			{
				GCTYDSCWXE=WIEVLYANFM("D",0)-SXSHBRJFIR;
...
XEMENNVXGV=function()
{
	...
	try
	{
		if (socket.id==undefined)
		{
			socket = io.connect(Maquina,{reconnection: true,  reconnectionDelay: 1000,
...
InicioPartidaOnline=function(Modalidad,Sala)
{
	...
	if (socket.id!=undefined)
	{
		socket.emit('PRHGDXYEOQ', socket.id, true,Modalidad,Sala,SFKLFIJOIO) ; //0: LXGKVFJUFT  1: Amigos
	}
}
...
Level.prototype.update=function(){
	...
	//deb.text=""
	//deb.text="Turno "+DVNRTQIWGL+" LUCRWXJMDR "+LUCRWXJMDR+" P: "+JVQXQGYDHH+" "+CNCCBHQJKL;
...
var ObjetoSala = function(AURESXCGMK,Sala,Cerrada,WWKVHIMWYD,BONQVNOXDP,QHHQPXWFNG,AvatarSprite,XJARYUVBCB,EJOYNSESRD,IRQGAFESIC,Amigo,OBTBPOULAV){
	this.AURESXCGMK=AURESXCGMK;
	this.Sala=Sala;
...
setEventHandlers = function () {
  ...
  socket.on('reconnect', function()
  {
	  ...
	  if (XPBCVAGCDQ.length>0) 
	  {
		  ...
		  {
			 console.log("JJJ "+ XPBCVAGCDQ[i].AURESXCGMK);
		  }
		  
		  
		  
		  
		  	for (var i=0;i<=NBEJWDVLLV.length-1;i++)
			{
				//console.log("Enviando el mensaje "+NBEJWDVLLV[0].OYJAHGADFQ+" de "+NBEJWDVLLV[0].OJPXTRQDKS);
				console.log("Comparo cliente "+NBEJWDVLLV[i].OJPXTRQDKS+" con "+XPBCVAGCDQ[TFSXFTYVGQ-1].AURESXCGMK+" y sala "+NBEJWDVLLV[i].DFBVDPETGO+" con "+DFBVDPETGO);
				if (NBEJWDVLLV[i].OJPXTRQDKS==XPBCVAGCDQ[TFSXFTYVGQ-1].AURESXCGMK && NBEJWDVLLV[i].DFBVDPETGO==DFBVDPETGO)
				{
  ...
  {
	  console.log(data);
	  KLDULSMKMA(data.data,data.RDBQUAVHEP)
	  return callback(data.OYJAHGADFQ);
  }
  )
  ...
  {
	  console.log(data);
	  onSocketCambiar7DesdeServer(data.UHTPGPRUJA,data.SJVCMXWVVI)
	  return callback(data.OYJAHGADFQ);
  }
  )
...
VSXSYEUABW=function(RPHWVBBGIM,VersionValida)
{
	var	TextoFormateado=EscribirTextoGuay(RPHWVBBGIM,40);
	
	var tw = YMHIHSNADE.add.tween(TextoFormateado).to( { alpha: 0 }, 1000, KYFMRVNHIQ, true,2000);
	tw.onComplete.add( function(){
		
		if (VersionValida==false)
		{
			VXEWRTCPMM=false;
			FHKOYDXPCL();
		}
...
GDRQVXOCBL=function(JugadorQueDesconecta,NombreDesconectado,posicionJugador)
{
	var LQPUBPQTJA;
	var i;
	var u;
	...
	log(true,"JUGADOR QUE DESCONECTA "+JugadorQueDesconecta);
		...
		//No se si sobra
	socket.emit('BKEMJDLQQM', DFBVDPETGO,JugadorQueDesconecta);
	
	for (var i=0;i<=XPBCVAGCDQ.length-1;i++)
	{
		if (XPBCVAGCDQ[i].AURESXCGMK==JugadorQueDesconecta)
		{
			XPBCVAGCDQ[i].XJARYUVBCB=true;
			KYDWXPGHVY.push( new UVYJLLYCYD ( (i+1),JugadorQueDesconecta ));
	...
	{		
		log(false,"JUGADORES DESCONECTADOS "+KYDWXPGHVY[i].SSUNXNEEHN);
	}
	...
	//Hacemos este retardo por si reconecta
...
VWYRRWMSJI=function(HOGPTRLBYH,NPYBJSHIFL)
{
	...
	FTBVKLREAE.anchor.setTo(0.5, 0.5);
		...
		var twBocadilloDesconectado = YMHIHSNADE.add.tween(FTBVKLREAE).to( { alpha: 1 }, 1000, KYFMRVNHIQ, true);
		twBocadilloDesconectado.onComplete.add(  function(){
				twBocadilloDesconectado = YMHIHSNADE.add.tween(FTBVKLREAE).to( { alpha: 0 }, 500, KYFMRVNHIQ, true,1000,0);
			}
		);
}
...
BCVWETKAKG=function(JugadorQueReconecta,NombreDesconectado,posicionJugador,NuevoID)
{
	...
	{
		if(BSFNXQXOTD[i]==NumeroJugador)
		{
			BSFNXQXOTD.splice(i,1);
			break;
		}
	}*/
...
//Level.prototype.Dibuja();
 ...
 RWANTPBSQE=function(MMIFVRWSYK)
 {
	...
	else
	{
		if (VEFCIYHBGC==false)
		{
			TextoFormateado=EscribirTextoGuay("Chat deshabilitado con "+XPBCVAGCDQ[MMIFVRWSYK-1].WWKVHIMWYD,40);
		}
		else
		{
			TextoFormateado=EscribirTextoGuay("Chat habilitado con "+XPBCVAGCDQ[MMIFVRWSYK-1].WWKVHIMWYD,40);
		}
	...
	var tw = YMHIHSNADE.add.tween(TextoFormateado).to( { alpha: 0 }, 1000, KYFMRVNHIQ, true,1000);
 ...
 NSIRYROROW = function(conectados,room,HGIFKEMXAD) {
	...
	if (XPBCVAGCDQ.length==4){
		 ...
		 if (XPBCVAGCDQ[3].AURESXCGMK==socket.id){ //Se crea la baraja
			//alert("b");
 ...
 DibujaJugadores=function(){
	...
	for (i=0;i<=XPBCVAGCDQ.length-1;i++){
		...
		if (IGPARKSECH(NumeroJugadorTemporal)==true)
		{
	 		log(true,"SSUNXNEEHN Temporal "+NumeroJugadorTemporal);
		
			UBWRXKMTQN=HHOPUSIABJ(NumeroJugadorTemporal);
			...
			if (UBWRXKMTQN=="L")
			{
				...
				if (i==0){
					...
					UWROMBGGSY.text=XPBCVAGCDQ[i].WWKVHIMWYD;
				...
				//	HUPALFWPFI.y=(y1text);
					...
					HUPALFWPFI.text=XPBCVAGCDQ[i].WWKVHIMWYD;
				...
				if (i==2){
					...
					BXYCJYDVRT.text=XPBCVAGCDQ[i].WWKVHIMWYD;
				...
				if (i==3){
					...
					IRAIEARIWA.text=XPBCVAGCDQ[i].WWKVHIMWYD;
...
KLDULSMKMA = function(Data,RDBQUAVHEP) {
//Tipo Idas o Vueltas
...
XWGQYYRRQO = function(UHTPGPRUJA,SJVCMXWVVI,BBSGITWRQX) {
 ...
 NOGEVUYFEW= function(UHTPGPRUJA,SJVCMXWVVI,turnoRecibido,SLGKDYQCAY,VMKDXVSQEY) {
	...
	{
		log(true,"DVNRTQIWGL DESDE SERVER "+UHTPGPRUJA+" con LUCRWXJMDR "+SLGKDYQCAY);
 ...
 onSocketCambiar7DesdeServer = function(UHTPGPRUJA,SJVCMXWVVI){
...
YUXWLFWVRD=function()
{
	if (EGPDVIEJEL==true)
	{
		...
		TKXPPGUPVV.add(3000, function(){
			//YMHIHSNADE.state.start("Level",true);	
		});
		TKXPPGUPVV.start();*/
