...
HUCLBWTDFV=function(UHTPGPRUJA,AWCCMSCOPO,OMDVKATFLQ)
{
	...
	if (OMDVKATFLQ==20 && STLEOJDWSU!="D")
	{
			...
			try
			{
				JQGKJGKCKJ.loadTexture(AKEULTNWHL(JQGKJGKCKJ), 0);
			}
			catch(e){}
			...
			try
			{
				IUQYICUQIU.loadTexture(AKEULTNWHL(IUQYICUQIU), 0);
			}
			catch(e){}
...
UYAKDBNLTD=function(){
	...
	//{
		...
		{
			KNVKGHHTYC[i].TNFAGGMKXD.loadTexture(AKEULTNWHL(KNVKGHHTYC[i].TNFAGGMKXD), 0);
		}
		for (i=0;i<=TWJONOKGHC.length-1;i++)
		{
			TWJONOKGHC[i].TNFAGGMKXD.loadTexture(AKEULTNWHL(TWJONOKGHC[i].TNFAGGMKXD), 0);
		}		
		for (i=0;i<=CBCNHFGWLU.length-1;i++)
		{
			CBCNHFGWLU[i].TNFAGGMKXD.loadTexture(AKEULTNWHL(CBCNHFGWLU[i].TNFAGGMKXD), 0);
		}		
	//}
	//catch(e){}
...
NAYBJTKIKA=function(TNFAGGMKXD){
	...
	if (HBFBHWJQSQ==false)
	{
		try
		{
			...
			TNFAGGMKXD.loadTexture(AKEULTNWHL(TNFAGGMKXD), 0);
		}
		catch(e){
...
function IHSCVAKSDJ(sprite) {
  ...
  var text;
	...
	if (KYLIAIETHX==false)
	{
		//INTERESANTE MensajeError2("Drag: Soy "+TFSXFTYVGQ+" Turno: "+DVNRTQIWGL+" Control:"+ESKTEXAVHU+" Reconectando: "+DUGDLNBOGK+" AGTXFNIKQD "+LUCRWXJMDR);
	}
	...
	if (HGFAUFDVDF()==true)
	{
		...
		timerpuntuacion.add(QJUYVEJMEB, function()
		{
			...
			if (JVQXQGYDHH==MFENMIFPUA || CNCCBHQJKL==MFENMIFPUA )
			{
				...
				var Copa=YMHIHSNADE.add.image(YMHIHSNADE.world.width+500,YMHIHSNADE.world.centerY, imagenFin);
				...
				BDEFDQUJJE = YMHIHSNADE.add.tween(text).to( { alpha: 0 }, TiempoDelay, KYFMRVNHIQ, true,TiempoAnimacion);
				BDEFDQUJJE.onComplete.add(
					function()
					{
							text.destroy();
							...
							CKOUBGTDUF = YMHIHSNADE.add.tween(text).to( { alpha: 0 }, TiempoDelay, KYFMRVNHIQ, true,TiempoAnimacion);
							CKOUBGTDUF.onComplete.add( 
...
UIYIUCHWUH=function(){
	...
	if (!DQKYYAPEEN.isRunning && !MJBCTSKUIB.isRunning && !UMEOIIEQQD.isRunning && !RWPENUDCCR.isRunning && !twMensajeError.isRunning)
	{
...
ComunBarajas=function()
{
	...
	DQKYYAPEEN=YMHIHSNADE.add.tween(BRQFVLOBSF);
	MJBCTSKUIB=YMHIHSNADE.add.tween(BRQFVLOBSF);
	UMEOIIEQQD=YMHIHSNADE.add.tween(BRQFVLOBSF);
	RWPENUDCCR=YMHIHSNADE.add.tween(BRQFVLOBSF);	
...
console.log("Enviándola");
	...
	var OYJAHGADFQ=Math.random()*100000;
	MQUGCIUQII=OYJAHGADFQ; //Para que no calcule otra vez
