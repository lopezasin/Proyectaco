...
QPGADJGDVQ=function(Jugador7)
{
	...
	if ( ( (EGPDVIEJEL==true && SNJCSVWFCC()) || EGPDVIEJEL==false ) && LUCRWXJMDR<=4    )
	{
		...
		||
		(LECTUTFGKY==true && TengoEl7(Jugador7)==true && Jugador7!=TFSXFTYVGQ && ESKTEXAVHU==TFSXFTYVGQ && FBSDMRLVEL[0].VVRCMEUNOJ>7 && EVAILWGGKX==true ) //Cambio por otro
		   ...
		   var UBWRXKMTQN=HHOPUSIABJ(Jugador7);
			...
			if (LSKPLOTYIB==true)
			{
				...
				if (EGPDVIEJEL==true)
				{
					...
					var KCPHFPSPLQ=new GXNRNMAAKW(DFBVDPETGO,Jugador7,XACEBCSJAB[i].TNFAGGMKXD.name, EVAILWGGKX,(JVQXQGYDHH+CNCCBHQJKL),XPBCVAGCDQ[Jugador7-1].OBTBPOULAV,XPBCVAGCDQ[Jugador7-1].AURESXCGMK, XPBCVAGCDQ[Jugador7-1].WWKVHIMWYD,
				...
				{				
					...
					FBSDMRLVEL.push(new TVYNYTCQYW(XACEBCSJAB[i].SGCSHJVERI,XACEBCSJAB[i].UUOPKETETA,XACEBCSJAB[i].HVCFEWNDRF,XACEBCSJAB[i].TNFAGGMKXD,5,XACEBCSJAB[i].jugador));
...
LMGBDQDXWP=function(){
	...
	if (VDKOVQXHON.length==0 || VDKOVQXHON.length==1)
	{
		...
		if (PPTWRPRLSI==true && TengoEl7(TFSXFTYVGQ)==true && FBSDMRLVEL[0].HVCFEWNDRF>7)
		{
			//YGTNAODFHW.revive();
			...
			if ( CIPQYKQBMR==true && SWIGVTVMDU==true && SRFFTPLVMJ("oros")==false) 
			{
				...
				if (TGTQPBEQFR=="oros")
				{
					if (EGPDVIEJEL==true)
					{
						...
						//1.0.9 WTERKRAPBE(UHTPGPRUJA,(JVQXQGYDHH+CNCCBHQJKL),EVAILWGGKX,0,false,true,false,false);
					}
					else
					{
				...
				else
				{
					if (EGPDVIEJEL==true)
					{
						...
						//1.0.9 WTERKRAPBE(UHTPGPRUJA,(JVQXQGYDHH+CNCCBHQJKL),EVAILWGGKX,0,true,false,false,false);
					}
					else
					{
...
QYYXUWQADX=function(TXLWIIINGQ){
	...
	var s = YMHIHSNADE.add.tween(FXGSEGRXKC[FXGSEGRXKC.length-1].TNFAGGMKXD);
		...
		HGMNXSIAPV=true;
		//console.log("OGGWQECXBK: sacando triunfo "+TXLWIIINGQ);
...
function IHSCVAKSDJ(sprite) {
  ...
  var text;
	...
	if (KYLIAIETHX==false)
	{
	 MensajeError2("Drag: Soy "+TFSXFTYVGQ+" Turno: "+DVNRTQIWGL+" Control:"+ESKTEXAVHU+" Reconectando: "+DUGDLNBOGK+" AGTXFNIKQD "+LUCRWXJMDR);
	}
	...
	if (1==1) //(!PQUFIBKJJH.isRunning && !LJRRHVSVKD.isRunning && !LCYAHFBDTT.isRunning && !NEPGDYQDEI.isRunning && !HHBPAROITT.isRunning && !EWXBTDLSHW.isRunning && !BDEFDQUJJE.isRunning && !CKOUBGTDUF.isRunning && !JLFYUAKDIJ.isRunning)
	{
		...
		*/
			...
			{
				//console.log("Antes validador reglas");
				if (XPSRYGDPNI(AGKVNAGGLB[i],TFSXFTYVGQ)==true)
				{
					//New
...
UFYWSHNLRM=function(XACEBCSJAB,STLEOJDWSU,SJVCMXWVVI,BBSGITWRQX,TXLWIIINGQ){
	...
	//console.log("OGGWQECXBK: ordenando");
	OGGWQECXBK(TXLWIIINGQ,false,"orden"); //Orden cartas
}
...
WWAYLOUPUQ=function() {
	...
	if (HGFAUFDVDF()==true ) //2.0.1 EGPDVIEJEL || EGPDVIEJEL==false
	{
		
		//Al borrar al finalizar, hay veces que se queda una tarea pillada. Paramos hasta detecta una baraja desde server
		...
		if (EVAILWGGKX==false || DUGDLNBOGK==true)
		{
			QJUYVEJMEB=0;
		}
		else
		...
		timerpuntuacion.add(QJUYVEJMEB, function()
		{
			...
			if (JVQXQGYDHH==MFENMIFPUA || CNCCBHQJKL==MFENMIFPUA )
			{
				...
				if (EGPDVIEJEL==true)
				{
					if (SNJCSVWFCC()==true)
					{
						socket.emit("UIEWYIIWIW", socket.id,DFBVDPETGO);
					}
				}
											...
											//d.style.zIndex=-1;
												...
												else
												{
													...
													{	
														...
														if (DUGDLNBOGK==false && HMVFLEYPXM.length==0)
														{
															//console.log("Nueva partida desde creartextopuntuacion");
															//console.log("Antes nueva partida");
															KPQSPDMKJE(SQIRFQMWSL); 
														}
														else
														{
															////console.log("OGGWQECXBK: nueva partida");
															//OGGWQECXBK(false); //Nueva partida
															...
															MMCDRVCGAV.loop(1000, function(){
																	...
																	if(HMVFLEYPXM.length>0 && HMVFLEYPXM[0].RHOLNWBOXR=="FFGKWCDVFE")
																	{
																		MMCDRVCGAV.stop();
																		OGGWQECXBK(false,true,false);
																		//LVEMXLIFKA(null,true);
																	}																
																} );
...
KPQSPDMKJE=function(SQIRFQMWSL)
{
	...
	{		
		...
		if (AGKVNAGGLB.length==0 && VDKOVQXHON.length==0 && KNVKGHHTYC.length==0 && CBCNHFGWLU.length==0 && XPBCVAGCDQ.length>=4)  //2.0.1 03/02/2018 Clave:Reparto
		{
		...
		if (VEFCIYHBGC==true)
		{
			...
			twUB1 = YMHIHSNADE.add.tween(GEFUYWBVWG[2]);
				...
				twUB1.onComplete.add( function(){
					JKKJHKHSWV(GEFUYWBVWG,CMWUMHNEHK,JKAHQMCRIV);
					}
				);
				twUB1.start();		
			}
...
JKKJHKHSWV=function(GEFUYWBVWG,MNABIEVSLA,JKAHQMCRIV){
...
//EfectosHacerPequenas2=function(){
		...
		twUB1.onComplete.add( function(){
			...
			KHCJWKUIYV(GEFUYWBVWG,MNABIEVSLA,JKAHQMCRIV);
...
KHCJWKUIYV=function(GEFUYWBVWG,MNABIEVSLA,JKAHQMCRIV){
	var NOFEDXHMLS;
	var	GHTWROCHVV;
...
//console.log("Enviándola");
	...
	var KCPHFPSPLQ=new GXNRNMAAKW(WUHRYVMYBE,DFBVDPETGO,RDBQUAVHEP,EVAILWGGKX,XPBCVAGCDQ,KXAWTTIEGG,OYJAHGADFQ,
